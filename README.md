# Adaptive Card Tab Sample - Microsoft Teams App

This project is an Adaptive Card Tab sample app, which demonstrates how to receive invoke messages from Teams client with examples responses. This sample is a work in progress and will continue to evolve as the feature develops and Bot Framework support is added.
...
3. Create a new _Web App_ with Windows App Service Plan and give it the name of your tab, the same you used when asked for URL in the Yeoman generator. In your case https://acprototype.azurewebsites.net.
...
11. Run the following command to set up the remote repository: `git remote add azure https://<username>@acprototype.scm.azurewebsites.net:443/acprototype.git`. You need to replace <username> with the username of the App Credentials you retrieved in _Deployment Credentials_. You can also copy the URL from _Options_ in the Azure Web App.
...
13. Wait until the deployment is completed and navigate to https://acprototype.azurewebsites.net/privacy.html to test that the web application is running
14. Done
